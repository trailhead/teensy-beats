...
void setup() {
  ...
  for (uint8_t x = 0; x < NUMCOLS; x++) {
    for (uint8_t y = 0; y < NUMROWS; y++) {
      keypad.setCallback(x, y, &toggle_note);
    }
  }
  ...
  wf_inst1.setToneType(WAVEFORM_TRIANGLE);
  ...
  pcm_inst1.setPath(0, "COSTCO~1.WAV");
  pcm_inst1.setPath(1, "154713~1.WAV");
  ...
  if (playing && ms >= next_tick) {
    ...
    for (inst = 0; inst < NUMINST; inst++) {
      if (notes[inst][tick / NUMCOLS][tick % NUMCOLS]) {
        ...
        if (inst == 2) {
          wf_inst1.playNote(notes[inst][tick / NUMCOLS][tick % NUMCOLS] - 1, 4, 1.0f);
        }
        if (inst == 3) {
          wf_inst1.playNote(notes[inst][tick / NUMCOLS][tick % NUMCOLS] - 1, 5, 1.0f);
        }
      }
    }
  }
